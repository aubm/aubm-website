...
export function Module({name, inject, modules = []}) {
  return Target => {
    ...
    Target.$angularModule = angular.isUndefined(inject) ? angular.module(name, modules.map(extractAngularModule)) : (typeof(inject) === "string") ? angular.module(inject) : inject;
...
export function RouteConfig({ path, resolve = {}, as = 'vm' }) {
  return Target => {
